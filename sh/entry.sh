...
# This script helps to automate the process of generating a JSON file from a collection of card images.
#
# Invocation is as follows:
#
#     ./entry.sh FILE_SPECIFIER
#
# where `FILE_SPECIFIER` is a wildcard path that resolves to one or more card image files, eg. "cards/*.png".
#
...
# Using Imagemagick, combine the tops of all the specified card images into one big image, and save it in the temporary
# directory.
...
# Create a sed script to transform the interleaved list (once we interleave the card names and images) to JSON.
SED_SCRIPT=$(cat <<'SED'
1~2 s/^.*$/        'name': '\0',/
2~2 s/^.*$/        'image': '\0',/
1~2 i \ \ \ \ {
...
# Use `paste` to interleave the two lists, then use the sed script to convert this to the appropriate JSON format.
